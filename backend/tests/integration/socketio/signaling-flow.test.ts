...
describe('socket.io signaling flow', () => {
  ...
  it('覆盖 bridge 鉴权、client:join、presence:nearby、webrtc:offer', async () => {
    ...
    const badRejected = await waitForEvent<{ reason: string }>(badBridge, 'auth:rejected');
    ...
    const aConnected = await waitForEvent<{ sessionId: string }>(clientA, 'connected');
    const bConnected = await waitForEvent<{ sessionId: string }>(clientB, 'connected');
    ...
    const aNearby = await waitForEvent<{ players: Array<{ sessionId: string }> }>(clientA, 'presence:nearby');
    ...
    const forwardedOffer = await waitForEvent<{ fromSessionId: string; data: { sdp: string } }>(clientB, 'webrtc:offer');
