    ...
    socket.on('message', (raw) => {
      ...
      switch (msg.type) {
        ...
        case 'connect': {
          ...
          if (!playerName) {
            safeSend(socket, { type: 'connectDenied', reason: 'missingPlayerName' });
            return;
          }
          ...
          if (existingSessionId) {
            // 尝试验证重连码
            const ok = code ? state.consumeReconnectCode(playerName, code, now) : false;
            if (!ok) {
              // 生成新的验证码并发送到游戏内
              const newCode = randomDigits(6);
              state.setReconnectCode(playerName, newCode, now + config.reconnectCodeTtlMs);
        ...
        case 'getAllPlayers': {
          ...
          const sessions = state.listOnlineClientSessions().map((s) => ({
            ...
            position: state.gamePlayersByName.get(s.playerName)?.position || null,
